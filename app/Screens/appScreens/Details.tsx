    ...
    async function fetchItemDetails() {
      try {
        const response = await axios.get(
          `https://8jcox47hg2.execute-api.us-east-2.amazonaws.com/dev/${itemId}`
        );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.containerText}>
        ...
        <Text style={styles.text}>{itemDetails?.text}</Text>
      </View>
      <View style={styles.footer}>
        <View style={styles.totalContainer}>
          <Text style={styles.footerText}>Total price </Text>
          <Text style={styles.newPrice}>${(itemDetails?.price * 0).toFixed(2)}</Text>
        </View>
